...
void Yolo::postprocess(cv::Mat& img)
{
    ...
    float width_factor = static_cast<float>(img.cols) / static_cast<float>(mInputDims.d[2]);
    float height_factor = static_cast<float>(img.rows) / static_cast<float>(mInputDims.d[3]);
